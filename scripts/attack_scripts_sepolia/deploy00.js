...
async function main() {
  ...
  console.log(
    "------------deploy00: 部署uniswapFactory和三种token ----------------------"
  );
  //部署 uniswapFactory 和三种token
  const uniswapFactory = await ethers.getContractFactory("UniswapV2Factory");
  const uniswap = await uniswapFactory.deploy(deployerAddress);
  const uniswapAddress = await uniswap.getAddress();
  console.log(`uniswap 部署在 ${uniswapAddress}`);
