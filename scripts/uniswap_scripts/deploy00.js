...
async function main() {
  ...
  const uniswap = await uniswapFactory.deploy(ownerAddress);
  const uniswapAddress = await uniswap.getAddress();
  console.log(`Uniswap Factory 部署地址 ${uniswapAddress}`);
  ...
  //铸造koko币、ac币
  //   await kokoToken.mint(ownerAddress, 2000);
  //   await acToken.mint(ownerAddress, 3000);
  //   let kokoBalance = await kokoToken.balanceOf(ownerAddress);
  //   let acBalance = await acToken.balanceOf(ownerAddress);
  //   console.log(
  ...
  //   let lpTokenBalance = await pair.balanceOf(ownerAddress);
  ...
  //   //查询 LP 流动币数量和 pair 交易对的储备量
  //   lpTokenBalance = await pair.balanceOf(ownerAddress);
